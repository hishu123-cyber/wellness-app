...
async function seed() {
  ...
  const schema = `
    ...
    CREATE TABLE IF NOT EXISTS ai_assessment_records (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id INTEGER NOT NULL,
      assessment_type TEXT NOT NULL DEFAULT 'comprehensive', -- questionnaire/tongue/voice/comprehensive
      primary_type TEXT,
      secondary_type TEXT,
      confidence REAL DEFAULT 0,
      detail TEXT, -- JSON: {tongue_result, voice_result, questionnaire_scores, ...}
      created_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY (user_id) REFERENCES users(id)
    );
  `;
